...
DoubleSolenoid buddySolenoid{5, 6};
...
void auton2(double m_LimelightTurnCmd, bool m_LimelightHasTarget, double limelightSpeed)
{
  ...
  else if (phase12 < timer.Get() && timer.Get() < phase13)
  {
    ...
    {
      flyWheelL.Set(ControlMode::Velocity, 0);
    }
...
void Robot::TeleopPeriodic()
{
  ...
  rightAxisY1 = operatorController.GetRawAxis(3);
  ...
  limelightSpeed = limelightShoot(limelightSpeed, m_LimelightHasTarget);
  cout << "limelight speed = " << limelightSpeed << endl;

  // Three Speed Shooter Control
  if (launcher(trenchSpeed, initSpeed, wallSpeed, limelightSpeed) == 0)
    flyWheelL.Set(0);
  else
    flyWheelL.Set(ControlMode::Velocity, launcher(trenchSpeed, initSpeed, wallSpeed, limelightSpeed));
  ...
  */
  ...
  if (rightTrigger0)
  {
    ...
    {
      //make this change depending on velocity
      conveyorMotor.Set(0.7);
      indexMotor.Set(.9);
    }
  }
  ...
  if (rightBumper1 && !lastRightJoyClick) // Lower Solenoid
  {
    if (intaketoggled)
    {
      intakeSolenoid.Set(DoubleSolenoid::Value::kForward);
