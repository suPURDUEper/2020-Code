...
void Robot::RobotInit()
{
  ...
  LpidController.SetSmartMotionAccel(kMaxAcc);
  ...
  RpidController.SetSmartMotionAccel(kMaxAcc);
...
void turnFunction(float distance)
{
  ...
  {
    LpidController.SetReference(desiredDistance, ControlType::kSmartMotion);
    RPidController.SetReference(-desiredDistance, ControlType::kSamrtMotion);
  }
}
...
void Robot::TeleopPeriodic()
{
  ...
  btnX = driveController.GetRawButton(0);

  SmartDashboard::PutBoolean("Beam", breakBeamBottomIn.Get());
  SmartDashboard::PutNumber("Velocity", TestMotor.GetSelectedSensorVelocity(kPIDLoopIdx));
